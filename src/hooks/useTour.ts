...
export const useTour = (): Tour => {
  ...
  const addSelectedArtist = useCallback(
    async (artists: Artist[]) => {
      await dispatchMiddleware(dispatch, {
        type: "add-artist",
        payload: { artists: artists, token: indState.token },
      });
  ...
  newRelatedArtists.forEach((element) => {
    ...
    if (!artistAlreadySelected) {
      if (state.relatedArtists.find((a) => a.id === element.id) === undefined) {
        ...
        relatedArtist.selectedArtistIDs =
          relatedArtist.selectedArtistIDs.concat([artist.id]);
      }
    }
  });
...
const addSelectedArtists = (
  ...
  for (var i = 0; i < artists.length; i++) {
    ...
    const newSelectedArtists = addSelectedArtistIfValid(
      state.selectedArtists,
      [],
      artists[i]
    );
    const shouldAddRelatedArtists =
      newSelectedArtists.length > state.selectedArtists.length;
...
const addSelectedArtistIfValid = (
  ...
  let artistDisliked: boolean =
    dislikedArtists.find((a) => a.id == newArtist.id) !== undefined;
...
const removeSelectedArtist = (
  ...
  state.relatedArtists.forEach((element) => {
    element.selectedArtistIDs = element.selectedArtistIDs.filter(
      (id) => id !== artist.id
    );
  });
